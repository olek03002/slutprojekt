...
from binance.client import Client as cryptoclient
...
class Economy(commands.Cog):
    ...
    @commands.command(aliases=["p"])  # Använder företaget "Binance"s API för att ta reda på nuvarande priser för olika kryptovalutor  
                                      # man skriver -price + symbolen för valutan, t.ex. BTC (Bitcoin), ETH (Etherium) eller DOGE (Dogecoin), alla valutor går
    async def price(self, ctx, token):
        api_key    = open("Trading/api.key").read()
        api_secret =  open("Trading/api_secret.key").read()
        binance = cryptoclient(api_key, api_secret)
        pris = binance.get_symbol_ticker(symbol=f"{token.upper()}USDT")["price"]
        await ctx.send("Price for " + f"{token.upper()} is " + pris.rstrip("0") + " Dollars")
